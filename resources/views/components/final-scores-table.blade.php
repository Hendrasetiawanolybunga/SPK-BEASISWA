            ...
            <div class="table-responsive">
                <table class="table table-hover align-middle mb-0">
                    <thead class="bg-light">
                        <tr>
                            <th class="py-3 px-4" style="width: 50px;"><i class="fas fa-medal me-1"></i> Ranking</th>
                            <th class="py-3 px-4"><i class="fas fa-user me-1"></i> Alternative</th>

                            @if ($method === 'moora')
                                <th class="py-3 px-4"><i class="fas fa-plus-circle text-success me-1"></i> Benefit Score
                                </th>
                                <th class="py-3 px-4"><i class="fas fa-minus-circle text-danger me-1"></i> Cost Score
                                </th>
                            @endif
                            ...
                            <th class="py-3 px-4">
                                ...
                                @else
                                    <i class="fas fa-chart-line text-primary me-1"></i> Final Deviation
                                @endif
                            </th>
                        </tr>
                    </thead>
                    <tbody>
                        @php
                            ...
                            if ($method === 'moora') {
                                ...
                                uasort($sortedScores, function ($a, $b) {
                                    $scoreA =
                                        is_array($a) && isset($a['final_deviation'])
                                            ? $a['final_deviation']
                                            : (is_numeric($a)
                                                ? $a
                                                : 0);
                                    $scoreB =
                                        is_array($b) && isset($b['final_deviation'])
                                            ? $b['final_deviation']
                                            : (is_numeric($b)
                                                ? $b
                                                : 0);
                        ...
                        @foreach ($sortedScores as $altName => $scoreData)
                            @php
                                $isTopRank = $rank <= 3;
                                $bgClass = $isTopRank
                                    ? 'table-' . ($method === 'moora' ? 'success' : 'warning') . ' bg-opacity-10'
                                    : '';
                                ...
                                if ($method === 'moora') {
                                    ...
                                    $finalScore =
                                        is_array($scoreData) && isset($scoreData['final_deviation'])
                                            ? $scoreData['final_deviation']
                                            : (is_numeric($scoreData)
                                                ? $scoreData
                                                : 0);
                                }
                            @endphp
                            ...
                            <tr class="{{ $bgClass }}">
                                <td class="text-center px-4">
                                    @if ($rank == 1)
                                        <span
                                            class="badge bg-{{ $method === 'moora' ? 'success' : 'warning' }} rounded-pill px-3 py-2">
                                            <i class="fas fa-crown me-1"></i> {{ $rank }}
                                        </span>
                                    @elseif($rank <= 3)
                                        <span
                                            class="badge bg-info text-dark rounded-pill px-3 py-2">{{ $rank }}</span>
                                    @else
                                        <span
                                            class="badge bg-secondary rounded-pill px-3 py-2">{{ $rank }}</span>
                                    @endif
                                </td>
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="progress flex-grow-1 me-2" style="height: 8px;">
                                                <div class="progress-bar bg-success" role="progressbar"
                                                    ...
                                                    aria-valuenow="{{ $benefitScore * 100 }}" aria-valuemin="0"
                                        ...
                                        <div class="d-flex align-items-center">
                                            <div class="progress flex-grow-1 me-2" style="height: 8px;">
                                                <div class="progress-bar bg-danger" role="progressbar"
                                                    ...
                                                    aria-valuenow="{{ $costScore * 100 }}" aria-valuemin="0"
                                                    aria-valuemax="100"></div>
                                            </div>
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="progress flex-grow-1 me-2" style="height: 8px;">
                                            <div class="progress-bar bg-{{ $method === 'moora' ? 'primary' : 'warning' }}"
                                                role="progressbar"
                                                style="width: {{ min(abs($finalScore) * 100, 100) }}%"
                                                aria-valuenow="{{ abs($finalScore) * 100 }}" aria-valuemin="0"
                                                aria-valuemax="100"></div>
                                        </div>
