...
<div class="container my-4">
    <h1 class="mb-4"><i class="fas fa-brain me-2"></i>Hasil Kombinasi <span class="text-primary">Naive Bayes</span> dan <span class="text-warning">MAIRCA</span></h1>
    ...
    {{-- Bagian MAIRCA --}}
    <section>
        <h2 class="mb-3"><i class="fas fa-tasks me-2"></i>Ranking <span class="text-warning">MAIRCA</span> (Hanya Alternatif Layak)</h2>
            ...
            <table class="table table-bordered table-striped table-hover align-middle mb-4">
                <thead class="table-dark">
                    <tr>
                        ...
                        <th><i class="fas fa-user"></i> Alternatif</th>
                        <th><i class="fas fa-chart-line"></i> Skor Deviasi (Semakin Kecil Semakin Baik)</th>
                    </tr>
                </thead>
                <tbody>
                    @php $rankNo = 1; @endphp
                    ...
                    <tr>
                        <td>{{ $rankNo++ }}</td>
                        <td>{{ $ranking['alt']->name }}</td>
                        <td>{{ number_format($ranking['score'], 6) }}</td>
                    </tr>
                    @endforeach
                </tbody>
            </table>
        ...
        <h3 class="mb-3"><i class="fas fa-book-reader me-2"></i>Detail Proses Perhitungan MAIRCA</h3>

        <div class="row g-3">
            <div class="col-md-6">
                <h5><i class="fas fa-th-list me-1"></i> Raw Matrix</h5>
                <pre class="bg-light p-3 rounded shadow-sm" style="max-height:300px; overflow:auto;">{{ print_r($maircaProses['raw_matrix'], true) }}</pre>
            </div>
            <div class="col-md-6">
                <h5><i class="fas fa-balance-scale me-1"></i> Normalized Matrix</h5>
                <pre class="bg-light p-3 rounded shadow-sm" style="max-height:300px; overflow:auto;">{{ print_r($maircaProses['normalized_matrix'], true) }}</pre>
            </div>
            <div class="col-md-6">
                <h5><i class="fas fa-weight-hanging me-1"></i> Ideal Weights</h5>
                <pre class="bg-light p-3 rounded shadow-sm" style="max-height:300px; overflow:auto;">{{ print_r($maircaProses['ideal_weights'], true) }}</pre>
            </div>
            <div class="col-md-6">
                <h5><i class="fas fa-chart-bar me-1"></i> Final Scores (Deviasi)</h5>
                <pre class="bg-light p-3 rounded shadow-sm" style="max-height:300px; overflow:auto;">{{ print_r($maircaProses['final_scores'], true) }}</pre>
            </div>
        </div>

        @else
        <p class="text-muted fs-5"><i class="fas fa-info-circle me-2"></i>Belum ada data untuk perhitungan MAIRCA, atau semua alternatif tidak layak.</p>
        @endif
    </section>
</div>
@endsection
